...
abstract class Deploy {
    ...
    /**
     * Info sent to stdout & stderr when executing commands
     */
    private $_command_output = array();
    ...
    private function execute() {
        try {
            ...
            $this->executeCommand('git pull ' . $this->_remote . ' ' . $this->_branch);

            // Secure the .git directory
            $this->executeCommand('chmod -R og-rx .git');
            ...
            $this->log( PHP_EOL.implode(PHP_EOL, $this->_command_output), 'DEBUG');
    ...
    private function executeCommand($command) {
        ...
        exec( $command.' 2>&1', $this->_command_output, $result);
        if (!empty($result)) throw new Exception('error on '.$command.' --->'.PHP_EOL.implode(PHP_EOL, $this->_command_output));
